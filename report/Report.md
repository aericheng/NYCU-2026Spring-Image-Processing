...
- **純車輛去模糊（影像 05 紅色計程車、08 速食店招牌）**：FFTformer 的去模糊真實、完全不含生成內容，但這些主體在原圖中本就部分可辨，修復前後的差距不大（上圖右）。
- **玻璃反射（影像 15 攝影師）**：反射疊影屬於 layer separation 問題而非模糊，FFTformer 與 SUPIR 都移除不了，臉仍然朦朧（上圖左）。
